...
public class OperationServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.setAttribute("pOne", new Person("김준수", 20, "남자"));
		request.setAttribute("pTwo", null); // 조회가 잘 안됐다는 가정하에 null값

		ArrayList<String> list1 = new ArrayList<>(); // 조회결과가 없다는 가정
		ArrayList<String> list2 = new ArrayList<>(); // 조회결과가 있다는 가정
		list2.add("ㅋㅋㅋ");
