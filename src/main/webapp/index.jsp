...
<body>
	...
	<p>
		JSP상에 표현하고자 하는 값을 \${XXX}의 형식으로 표현해서 작성하는 것
	...
	<!-- 
		* JSP를 이루는 구성인자
		...
		2. 지시어 : JSP페이지 정보에 대한 내용을 표현한다거나 또 다른 페이지를 포함할 때 사용
		          JSP 기능을 확장시키는 라이브러리 등록할 때도 사용
		EX) page지시어, include지시어, taglib지시어
		
		3. 액션태그 : xml기술을 이용해서 기존의 jsp문법을 확장하는 기능을 제공하는 태그
		>> 표준 액션 태그 (Standard Action Tag) 
		   JSP에서 바로 사용 가능 (별도의 연동 X), 모든 태그 명 앞에 jsp : 접두어 붙음 
		
		>> 커스텀 액션 태그 (Custom Action Tag)
		   JSP에서 바로 사용 불가능 (별도의 라이브러리 연동 필요), 모든 태그 명 앞에 jsp : 외의 접두어 붙음 (종류는 다양)
